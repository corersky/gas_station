...
public class Cash_Pay_Activity extends BaseActivity {
	...
	TextView jiayou_qxjy_type=null;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		((GasStationApplication) getApplication()).tempActivity.add(Cash_Pay_Activity.this);
	...
	private void init() {
		...
		jiayou_qxjy_type=(TextView) findViewById(R.id.jiayou_qxjy_type);
		jiayou_qxjy_type.setText("使用现金券");
	}
	...
	private void queryTicket() {
			...
			public void handleMessage(Message msg) {
				...
				case 100:
					...
					while(it.hasNext()) {
						...
						final int pos=i;
						View view=LayoutInflater.from(Cash_Pay_Activity.this).inflate(R.layout.view_qxjy, null);
						...
						if(getIntent().getExtras().getInt("offer_type_id")==1) {
							if(entry.getKey().toString().substring(7).equals("10")) {
								qxjy_choice_text.setText("仅抵扣当月4元");
							}
							else if(entry.getKey().toString().substring(7).equals("20")) {
								qxjy_choice_text.setText("仅抵扣当月8元");
							}
							else if(entry.getKey().toString().substring(7).equals("30")) {
								qxjy_choice_text.setText("仅抵扣当月12元");
							}
						}
						else {
							if(entry.getKey().toString().substring(7).equals("10")) {
								qxjy_choice_text.setText("次月自动抵扣4元，连续3个月");
							}
							else if(entry.getKey().toString().substring(7).equals("20")) {
								qxjy_choice_text.setText("次月自动抵扣8元，连续3个月");
							}
							else if(entry.getKey().toString().substring(7).equals("30")) {
								qxjy_choice_text.setText("次月自动抵扣12元，连续3个月");
							}
						}
						if(i==map.size()-1) {
							qxjy_choice_num=i;
		...
		new Thread(new Runnable() {
			...
			public void run() {
				// TODO Auto-generated method stub
				LinkedList<String> wholeUrl=Util.getWholeUrl(Cash_Pay_Activity.this);
				Message m=new Message();
				int num=0;
				boolean flag=true;
				String currentUsedUrl="";
				try {
					currentUsedUrl=((GasStationApplication) getApplicationContext()).AreaUrl.equals("")?Util.getWholeUrl(Cash_Pay_Activity.this).get(0):((GasStationApplication) getApplicationContext()).AreaUrl;
				} catch(Exception e) {
					currentUsedUrl=((GasStationApplication) getApplicationContext()).COMMONURL[0];
				}
				while(flag) {
					try {
						ArrayList<String> list=Util.getUserInfo(Cash_Pay_Activity.this);
						
						StrategyManager strategyManager=GetWebDate.getHessionFactiory(Cash_Pay_Activity.this).create(StrategyManager.class, currentUsedUrl+"/hessian/strategyManager", getClassLoader());
						HashMap result=strategyManager.queryXjTicket(Long.parseLong(list.get(0)), list.get(1), getIntent().getExtras().getInt("cost"));
						m.what=100;
						m.obj=result;
						flag=false;
			...
			public void run() {
				...
				while(flag) {
					try {
						...
						PublicManager publicManager=GetWebDate.getHessionFactiory(Cash_Pay_Activity.this).create(PublicManager.class, currentUsedUrl+"/hessian/publicManager", getClassLoader());
			...
			public void run() {
				...
				while(flag) {
					try {
						...
						Map result=strategyManager.saveXjqOrder(String.valueOf(temp_time), Long.parseLong(list.get(0)), Long.parseLong(getIntent().getExtras().getString("offerId")), 
	...
	protected void onDestroy() {
		...
		((GasStationApplication) getApplication()).tempActivity.remove(Cash_Pay_Activity.this);
	}
}
