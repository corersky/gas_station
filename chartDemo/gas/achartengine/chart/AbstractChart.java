...
import gas.achartengine.renderer.XYMultipleSeriesRenderer.Orientation;
import gas.achartengine.util.MathHelper;
...
public abstract class AbstractChart implements Serializable {
  ...
  protected int drawLegend(Canvas canvas, DefaultRenderer renderer, String[] titles, int left,
    ...
    if (renderer.isShowLegend()) {
      float currentX = left;
      //调整高度
      float currentY = y + height - legendSize + size - renderer.getLegendHightRejust();
  ...
  protected void drawLabel(Canvas canvas, String labelText, DefaultRenderer renderer,
	  ...
	  //修改label和饼图上字必须同时显示的问题，可单独显示饼图上的文字  || (!renderer.isShowLabels() && !line)
    if (renderer.isShowLabels() || (!renderer.isShowLabels() && !line)) {
      paint.setColor(color);
