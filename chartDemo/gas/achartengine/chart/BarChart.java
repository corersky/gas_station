...
import gas.achartengine.model.XYSeries;
  ...
  protected void drawChartValuesText(Canvas canvas, XYSeries series, SimpleSeriesRenderer renderer,
    ...
    for (int i = 0; i < points.length; i += 2) {
      ...
      if (!isNullValue(value)) {
        ...
        //修改柱图y轴数值显示位置（往左微调）x - 6
        if (value >= 0) {
          drawText(canvas, getLabel(value), x - 6, points[i + 1] - renderer.getChartValuesSpacing(),
              paint, 0);
        } else {
          drawText(canvas, getLabel(value), x - 6, points[i + 1] + renderer.getChartValuesTextSize()
